								...
								<c:when test="${not empty userList}">
									...
									<c:forEach items="${userList}" var="user" varStatus="vs">
										<tr>
											...
											<td class="center">${user.endTime }</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${jurisdiction.email == 1 }">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									...
									<c:if test="${jurisdiction.email == 1 }"><a title="批量发送电子邮件" class="btn btn-mini btn-info" onclick="makeAll('确定要给选中的用户发送邮件吗?');"><i class="ace-icon fa fa-envelope bigger-120"></i></a></c:if>
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
 	...
 	<script type="text/javascript">
		...
		function editUser(userId){
			 ...
			 diag.Drag=true;
			 diag.Title ="会员资料";
			 diag.URL = '<%=basePath%>happuser/goEditU.do?userId='+userId;
			 diag.Width = 600;
			 diag.Height = 419;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function delUser(userId,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>happuser/deleteU.do?userId="+userId+"&tm="+new Date().getTime();
		...
		function toExcel(){
			...
			window.location.href='<%=basePath%>happuser/excel.do?keywords='+keywords+'&lastLoginStart='+lastLoginStart+'&lastLoginEnd='+lastLoginEnd+'&roleId='+roleId+'&status='+status;
		}
		</script>
</html>
