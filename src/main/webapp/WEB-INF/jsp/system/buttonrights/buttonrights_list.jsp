...
<body class="no-skin">
							...
							<table id="dynamic-table" class="table table-striped table-bordered table-hover" style="margin-top:7px;">
								...
								<c:choose>
									<c:when test="${not empty roleListZ}">
										...
										<td class='center' id="roleNameTd${var.roleId }">${var.roleName }</td>
										<c:if test="${jurisdiction.edit == 1 }">
											<!-- 按钮循环 -->
											<c:forEach items="${buttonlist}" var="fhbutton" varStatus="vsb">
												<!-- 关联表循环 -->
												<c:forEach items="${roleFhbuttonlist}" var="varRb" varStatus="vsRb">
													<c:if test="${var.roleId == varRb.roleId && fhbutton.fhbuttonId == varRb.buttonId }">
														<c:set value="1" var="rbvalue"></c:set>
													</c:if>
												</c:forEach>
												<td class='center' style="height: 20px;">
													<label>
														<input name="switch-field-1" onclick="upRb('${var.roleId}','${fhbutton.fhbuttonId}')" class="ace ace-switch ace-switch-3" type="checkbox" <c:if test="${rbvalue == 1 }">checked="checked"</c:if> >
														<span class="lbl"></span>
													</label>
												</td>
	...
	<script type="text/javascript">
		...
		//处理按钮点击
		function upRb(roleId,fhbuttonId){
			$.ajax({
				type: "POST",
				url: "<%=basePath%>buttonrights/upRb.do?roleId="+roleId+"&buttonId="+fhbuttonId+"&guid="+new Date().getTime(),
		    	data: encodeURI(""),
				dataType:'json',
