							...
							<tr>
								...
								<td>
									<select name="fhtype" id="fhtype" data-placeholder="请选择数据库"  style="vertical-align:top;width: 87px;" onchange="selectType(this.value)">
										<c:choose>  
										   <c:when test="${pd.fhtype eq 'single' }">     
										   		<option value="single" selected="selected">单表</option>
										   </c:when>  
										   <c:otherwise>  
										...
										<c:choose>  
										   <c:when test="${pd.fhtype eq 'tree' }">     
										   		<option value="tree" selected="selected">树形</option>
										   </c:when>  
										   <c:otherwise>  
										...
										<c:choose>  
										   <c:when test="${pd.fhtype eq 'fathertable' }">     
										   		<option value="fathertable" selected="selected">主表</option>
										   </c:when>  
										   <c:otherwise>  
										...
										<c:choose>  
										   <c:when test="${pd.fhtype eq 'sontable' }">     
										   		<option value="sontable" selected="selected">明细表</option>
										   </c:when>  
										   <c:otherwise>  
							...
							<tr>
								...
								<td style="padding-left: 10px;">
									<select name="faobjectid" id="faobjectid" data-placeholder="请选择" disabled="disabled" style="vertical-align:top;width:150px;background-color: #F5F5F5;" onchange="selectFa(this.value)">
										...
										<option value="${var.createcodeId }">${var.objectName}</option>
										</c:forEach>
									</select>
								</td>
							</tr>
...
<!-- /.main-container -->	
		...
		$(top.hangge());//关闭加载状态
		$(reductionFields());
		<c:if test="${pd.fhtype eq 'sontable' }">     
			selectType('sontable');
		</c:if>  
		//修改时还原属性列表
		function reductionFields(){
			...
			if('edit' == msg){
				...
				var fieldarray = nowarField.split('Q149156999');
