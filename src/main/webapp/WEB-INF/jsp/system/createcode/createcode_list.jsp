...
<body class="no-skin">
							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.createcodeId}" class="ace" /><span class="lbl"></span></label>
											</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-success" title="编辑" onclick="productCode('${var.createcodeId}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${jurisdiction.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="del('${var.createcodeId}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<c:if test="${jurisdiction.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="productCode('${var.createcodeId}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${jurisdiction.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${var.createcodeId}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
	...
	<script type="text/javascript">
		...
		//启动代码生成器
		function productCode(createcodeId){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="代码生成器";
			 diag.URL = '<%=basePath%>createCode/goProductCode.do?createcodeId='+createcodeId;
			 diag.Width = 800;
			 diag.Height = 500;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>createCode/delete.do?createcodeId="+Id+"&tm="+new Date().getTime();
