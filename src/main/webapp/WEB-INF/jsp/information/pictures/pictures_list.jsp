...
<body class="no-skin">
								...
								<c:forEach items="${varList}" var="var" varStatus="vs">
									<tr>
										<td class='center' style="width: 30px;">
											<label><input type='checkbox' name='ids' class="ace" value="${var.picturesId}" /><span class="lbl"></span></label>
										</td>
										...
										<td class="center">${var.picturesId}</td>
										<td class="center">${var.createtTme}</td>
										<td class="center">${var.masterId}</td>
										...
										<span class="label label-large label-grey arrowed-in-right arrowed-in"><i class="ace-icon fa fa-lock" title="无权限"></i></span>
											...
											<a style="cursor:pointer;" class="green" onclick="edit('${var.picturesId}');" title="编辑">
												<i class="ace-icon fa fa-pencil bigger-130"></i>
											</a>
											</c:if>
											&nbsp;
											<c:if test="${jurisdiction.del == 1 }">
											<a style="cursor:pointer;" class="red" onclick="del('${var.picturesId}','${var.path}');" title="删除">
	...
	<script type="text/javascript">
		...
		function del(Id,path){
			...
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>pictures/delete.do?picturesId="+Id+"&path="+path+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>pictures/goEdit.do?picturesId='+Id;
			 diag.Width = 600;
			 diag.Height = 465;
			 diag.CancelEvent = function(){ //关闭事件
