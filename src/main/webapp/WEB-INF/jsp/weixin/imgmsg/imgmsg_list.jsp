										...
										<c:forEach items="${varList}" var="var" varStatus="vs">
											<tr>
												<td class='center' style="width: 30px;">
													<label><input type='checkbox' name='ids' class="ace" value="${var.imgmsgId}" /><span class="lbl"></span></label>
												</td>
												...
												<td class="center" style="width: 100px;">
													...
													<a style="cursor:pointer;" class="green" onclick="edit('${var.imgmsgId}');" title="编辑">
													<i class="ace-icon fa fa-pencil bigger-130"></i>
													</a>
													</c:if>
													<c:if test="${jurisdiction.del == 1 }">
													<a style="cursor:pointer;" class="red" onclick="del('${var.imgmsgId}');" title="删除">
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>					
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>imgmsg/delete.do?imgmsgId="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>imgmsg/goEdit.do?imgmsgId='+Id;
			 diag.Width = 860;
			 diag.Height = 680;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
