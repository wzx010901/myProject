									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center' style="width: 30px;">
												<label><input type='checkbox' name='ids' class="ace" value="${var.commandId}" /><span class="lbl"></span></label>
											</td>
											...
											<td class="center">
												<div class='hidden-phone visible-desktop btn-group'>
													...
													<a style="cursor:pointer;" class="green" onclick="edit('${var.commandId}');" title="编辑">
														<i class="ace-icon fa fa-pencil bigger-130"></i>
													</a>
													</c:if>
													<c:if test="${jurisdiction.del == 1 }">
													<a style="cursor:pointer;" class="red" onclick="del('${var.commandId}');" title="删除">
	...
	<script type="text/javascript">
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>command/delete.do?commandId="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>command/goEdit.do?commandId='+Id;
			 diag.Width = 650;
			 diag.Height = 279;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
