...
/**
 * 说明：爬取网页 创建人：FH Q313596790 修改时间：2016年3月24日
 * 
 * @version
 */
public class GetWeb {
	...
	public static String getHtmlCode(String httpUrl) throws IOException {
		...
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(url.openStream(), "utf-8"));) {// 获得当前url的字节流（缓冲）
			String input;
...
//		reader.close(); // 关闭缓冲区
	...
	public static List<String> getImagePathList(String httpUrl) throws IOException {
		...
		while (matcher.find()) {
			...
			String imgsrc = (quote == null || quote.trim().length() == 0) ? matcher.group(4).split("\\s+")[0]
					: matcher.group(4);
