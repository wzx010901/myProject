...
public class DictionariesService implements DictionariesManager{
	...
	public PageData findByEncoding(PageData pd)throws Exception{
		return (PageData)dao.findForObject("DictionariesMapper.findByEncoding", pd);
	}
	...
	public List<Dictionaries> listAllDict(String parentId) throws Exception {
		...
		for(Dictionaries dict : dictList){
			dict.setTreeUrl("dictionaries/list.do?dictionariesId="+dict.getDictionariesId());
			dict.setSubDict(this.listAllDict(dict.getDictionariesId()));
