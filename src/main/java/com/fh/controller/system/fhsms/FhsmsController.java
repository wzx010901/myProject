...
public class FhsmsController extends BaseController {
		...
		if(null != username && !"".equals(username)){
			...
			try {
				...
				for(int i=0;i<arrUsername.length;i++){
					pd.put("sanmeId", this.get32UUID());					//共同ID
					pd.put("sendTime", DateUtil.getTime());					//发送时间
					pd.put("fhsmsId", this.get32UUID());					//主键1
					pd.put("type", "2");									//类型2：发信
					pd.put("fromUsername", Jurisdiction.getUsername());		//发信人
					pd.put("toUsername", arrUsername[i]);					//收信人
					fhsmsService.save(pd);									//存入发信
					pd.put("fhsmsId", this.get32UUID());					//主键2
					pd.put("type", "1");									//类型1：收信
					pd.put("fromUsername", arrUsername[i]);					//发信人
					pd.put("toUsername", Jurisdiction.getUsername());		//收信人
	...
	public ModelAndView list(Page page) throws Exception{
		...
		pd.put("fromUsername", Jurisdiction.getUsername()); 	//当前用户名
		page.setPd(pd);	
	...
	public Object deleteAll() throws Exception{
		...
		if(null != dataIds && !"".equals(dataIds)){
			...
			fhsmsService.deleteAll(arrayDataIds);
			pd.put("msg", "ok");
		}else{
			pd.put("msg", "no");
		}
