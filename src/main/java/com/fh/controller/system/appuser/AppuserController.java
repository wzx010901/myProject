	...
	public Object deleteAllU() {
		...
		try {
			...
			if(null != userIds && !"".equals(userIds)){
				...
				appuserService.deleteAllU(ArrayUserIds);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
		...
		try{
			if(Jurisdiction.buttonJurisdiction(menuUrl, "find")){	
				...
				for(int i=0;i<userList.size();i++){
					...
					vpd.put("var6", userList.get(i).getString("roleName"));		//6
					vpd.put("var7", userList.get(i).getString("email"));		//7
					vpd.put("var8", userList.get(i).getString("lastLogin"));	//8
					vpd.put("var9", userList.get(i).getString("endTime"));		//9
