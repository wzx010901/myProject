...
/**
 * 类名称：ToolController 系统工具 创建人：WZX Q149156999 修改时间：2015年11月23日
 * 
 * @version
 */
@Controller
...
public class ToolController extends BaseController {
	...
	public Object getDistance() {
		...
		try {
			distance = MapDistance.getDistance(pd.getString("coordinateY"), pd.getString("coordinateX"),
					pd.getString("coordinateY2"), pd.getString("coordinateX2"));
		} catch (Exception e) {
			errInfo = "error";
		}
		map.put("result", errInfo); // 返回结果
		map.put("distance", distance); // 距离R
		return AppUtil.returnObject(new PageData(), map);
	}
	...
	public ModelAndView fusionchartsdemo() throws Exception {
		...
		String strXML = "<graph caption='前12个月订单销量柱状图' xAxisName='月份' yAxisName='值' decimalPrecision='0' formatNumberScale='0'>"
				...
				+ "<set name='2012-07' value='0' color='AFD8F8'/>" + "<set name='2012-06' value='0' color='AFD8F8'/>"
				+ "</graph>";
