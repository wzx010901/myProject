...
/**
 * 说明：数据库管理(备份和还原) 创建人：WZX Q149156999 创建时间：2016-03-30
 */
@Controller
...
public class BRdbController extends BaseController {
	...
	public ModelAndView listAllTable() throws Exception {
		...
		if (!Jurisdiction.buttonJurisdiction(menuUrlb, "find")) {
			return null;
	...
	public Object backupAll() {
		...
		try {
			...
			if (Tools.notEmpty(kackupPath) && !Objects.equals("errer", kackupPath)) {
				...
				pd.put("remark", username + "备份全库操作"); // 备注
				pd.put("msg", "ok");
				try {
					brdbService.save(pd);
				} catch (Exception e) {
	...
	public Object backupTable() {
		...
		try {
			...
			if (Tools.notEmpty(kackupPath) && !Objects.equals("errer", kackupPath)) {
				...
				pd.put("remark", username + "备份单表"); // 备注
				pd.put("msg", "ok");
				try {
					brdbService.save(pd);
				} catch (Exception e) {
